...
if(isset($_POST['firstname']) &&
   ...
   $username = strtolower(substr($_POST['firstname'],0,1)).strtolower($_POST['lastname']);
   ...
   if (empty($name)) {
	  ...
	  */
      ...
      if($stmt->rowCount() > 0){
		...
		$cheminFichier = "comptes/$name.txt";
		file_put_contents($cheminFichier, $contenuFichier);
      	...
      	if (isset($new_img_name)) {
            ...
            $sql = "INSERT INTO users
                    (name, username, password, role, p_p)
                    VALUES (?,?,?,?,?)";
            $stmt = $conn->prepare($sql);
            $stmt->execute([$name, $username, $password, $role, $new_img_name]);
      	}else {
            # inserting data into database
            $sql = "INSERT INTO users
	...
	header("Location: backoffice.php?page=addaccount");
   	exit;
}
